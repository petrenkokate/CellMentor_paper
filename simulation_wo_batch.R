...
run_simulation <- function(sim_index, params) {
  SIM_NUMBER = paste0('_wobatch_final_', sim_index)
  set.seed(100)
  ...
  file_seu <- paste0('save_data_/simulation_6tools', SIM_NUMBER, '.qs')
...
qsave(results_list, 
      paste0('save_data/final_all_simulations_results_wobatch', format(Sys.time(), "%Y%m%d_%H%M"), '.qs'))
