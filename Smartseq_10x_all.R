...
# Get intersecting tissues
common_tissues <- intersect(
  droplet@colData$tissue %>% unique(),
  smartseq@colData$tissue %>% unique()
)

print(paste("Found", length(common_tissues), "common tissues:"))
print(common_tissues)

# Create directory for results if it doesn't exist
if (!dir.exists('save_data_v3')) {
  dir.create('save_data_v3', recursive = TRUE)
}

# Function to check data quality and cell counts
check_tissue_data <- function(tissue_name, droplet_data, smartseq_data) {
  # Subset data for the tissue
  tissue_10x <- droplet_data[, droplet_data@colData$tissue == tissue_name & 
                               !is.na(droplet_data@colData$cell_ontology_class)]
  tissue_smartseq <- smartseq_data[, smartseq_data@colData$tissue == tissue_name & 
                                     !is.na(smartseq_data@colData$cell_ontology_class)]
  
  # Get cell type counts
  cell_types_10x <- table(tissue_10x@colData$cell_ontology_class)
  cell_types_smartseq <- table(tissue_smartseq@colData$cell_ontology_class)
  
  # Find common cell types
  common_cell_types <- intersect(names(cell_types_10x), names(cell_types_smartseq))
  
  return(list(
    tissue = tissue_name,
    n_cells_10x = ncol(tissue_10x),
    n_cells_smartseq = ncol(tissue_smartseq),
    n_genes_10x = nrow(tissue_10x),
    n_genes_smartseq = nrow(tissue_smartseq),
    cell_types_10x = length(cell_types_10x),
    cell_types_smartseq = length(cell_types_smartseq),
    common_cell_types = length(common_cell_types),
    cell_type_names = common_cell_types,
    min_cells_per_type_10x = if(length(cell_types_10x) > 0) min(cell_types_10x) else 0,
    min_cells_per_type_smartseq = if(length(cell_types_smartseq) > 0) min(cell_types_smartseq) else 0
  ))
}

# Function to run CellMentor analysis for a single tissue
run_tissue_analysis <- function(tissue_name, droplet_data, smartseq_data, 
                                num_cores = 20, verbose = TRUE) {
  ...
  tryCatch({
    ...
    # Subset data for the tissue
    cat("Subsetting data for", tissue_name, "...\n")
    tissue_10x <- droplet_data[, droplet_data@colData$tissue == tissue_name & 
                                 !is.na(droplet_data@colData$cell_ontology_class)]
    tissue_smartseq <- smartseq_data[, smartseq_data@colData$tissue == tissue_name & 
                                       !is.na(smartseq_data@colData$cell_ontology_class)]
    ...
    # Check data quality
    data_info <- check_tissue_data(tissue_name, droplet_data, smartseq_data)
    cat("Data summary:\n")
    cat("  10x cells:", data_info$n_cells_10x, "| SmartSeq cells:", data_info$n_cells_smartseq, "\n")
    cat("  Common cell types:", data_info$common_cell_types, "\n")
    cat("  Cell types:", paste(data_info$cell_type_names, collapse = ", "), "\n")
    
    # Skip if insufficient data
    if (data_info$n_cells_10x < 50 || data_info$n_cells_smartseq < 50 || 
        data_info$common_cell_types < 2) {
      cat("SKIPPING", tissue_name, "- Insufficient data\n")
      return(list(
        tissue = tissue_name,
        status = "skipped",
        results = NULL,
    ...
    # Extract matrices
    lung_matrix_ref <- assay(tissue_10x)
    lung_matrix_test <- assay(tissue_smartseq)
    rownames(lung_matrix_ref) <- make.unique(rownames(lung_matrix_ref))
    rownames(lung_matrix_test) <- make.unique(rownames(lung_matrix_test))
    
    seu_ref <- CreateSeuratObject(counts = lung_matrix_ref, meta.data = as.data.frame(colData(tissue_10x)))
    seu_test <- CreateSeuratObject(counts = lung_matrix_test, meta.data = as.data.frame(colData(tissue_smartseq)))
    seu_test <- NormalizeData(seu_test)
    seu_ref <- NormalizeData(seu_ref)
    seu_test$celltype <- seu_test$cell_ontology_class
    seu_ref$celltype <- seu_ref$cell_ontology_class
    seu_ref$Group <- seu_ref$celltype
    seu_test$Group <- seu_test$celltype

    #Check if results already exist
    file_path <- paste0('save_data_v3/simulation_method_', tissue_name, '_10x_smartseq.qs')

    if (file.exists(file_path)) {
      cat("RESULTS ALREADY EXIST FOR", tissue_name, ". LOADING FILE...\n")
      optimal_params <- qread(file_path)
    }
    else{
      # Create CellMentor object
      cat("Creating CellMentor object...\n")
      tic()
      object <- CreateCSFNMFobject(
        ref_matrix = lung_matrix_ref,
        ref_celltype = tissue_10x@colData$cell_ontology_class,
        data_matrix = lung_matrix_test,
        verbose = verbose
      )
      time_create <- toc(quiet = TRUE)
      cat("Object creation time:", round(time_create$toc - time_create$tic, 2), "seconds\n")

      # Run parameter optimization
      cat("Running CellMentor parameter optimization...\n")
      tic()
      ...
      time_analysis <- toc(quiet = TRUE)
      cat("Analysis time:", round(time_analysis$toc - time_analysis$tic, 2), "seconds\n")
    }
    ...
    common_genes <- intersect(rownames(seu_ref), rownames(seu_test2))

    # Subset and reorder both objects
    seu_ref <- seu_ref[common_genes, ]
    seu_test2 <- seu_test2[common_genes, ]
    ...
    create_weight_matrix <- function(expr_matrix, marker_genes, sigma = 1) {
      ...
      marker_expr <- expr_matrix[rownames(expr_matrix) %in% marker_genes, , drop = FALSE]
      
      # Transpose to have cells as rows (as required by dist)
      cell_expr <- t(marker_expr)  # Now rows = cells, columns = genes
      
      # Compute pairwise Euclidean distance matrix
      dist_matrix <- as.matrix(dist(cell_expr, method = "euclidean"))
      
      # Apply heat kernel
      Q <- exp(-dist_matrix^2 / sigma)
      
      # Optional: zero out diagonal if you want no self-weight
      diag(Q) <- 0
    ...
    seu_ref$rssNMF_clusters <- seu_ref$RNA_snn_res.0.2
    ...
    # Ensure the gene names match and are in the same order
    common_genes <- intersect(rownames(W_ref), rownames(test_data))
    if (length(common_genes) < 10) {
      stop("Too few overlapping genes between W_ref and test_data.")
    }
    
    W_ref_aligned <- W_ref[common_genes, , drop = FALSE]
    test_data_aligned <- test_data[common_genes, , drop = FALSE]
    
    # Now run projection safely
    H_test <- project_rssNMF(test_data_aligned, W_ref_aligned)
    ...
    seu_test$rssNMF_clusters <- seu_test$RNA_snn_res.0.2
    ...
    seu_ref$LIGER_clusters <- seu_ref$RNA_snn_res.0.2
    ...
    seu_test$LIGER_clusters <- seu_test$RNA_snn_res.0.2
    ...
    seu_test$scCoGAPS_clusters <- seu_test$RNA_snn_res.0.2
...
", output_file))
    ...
    seu_test$cassl_clusters <- seu_test$RNA_snn_res.0.2
    
    qsave(seu_test, 
          paste0('save_data_v3/seu_test_', tissue_name, '.qs'))
    qsave(seu_ref, 
          paste0('save_data_v3/seu_ref_', tissue_name, '.qs'))
    
    cat("✓ COMPLETED", tissue_name, "successfully!\n")
    ...
    return(list(
      ...
      results = NULL,
      error = e$message,
      data_info = if(exists("data_info")) data_info else NULL
    ))
  })
}

# Check data quality for all tissues first
cat("Checking data quality for all tissues...\n")
tissue_summary <- map_dfr(common_tissues, function(tissue) {
  info <- check_tissue_data(tissue, droplet, smartseq)
  data.frame(info)
})

print(tissue_summary)

# Filter tissues with sufficient data
sufficient_tissues <- tissue_summary %>%
  dplyr::filter(n_cells_10x >= 50, n_cells_smartseq >= 50, common_cell_types >= 2) %>%
  pull(tissue) %>% unique()

cat("\nTissues with sufficient data for analysis:\n")
print(sufficient_tissues)

# Run analysis for all sufficient tissues
cat("\n\nStarting analysis for", length(sufficient_tissues), "tissues...\n")

# Initialize results storage
all_results <- list()

# Process each tissue
for (i in seq_along(sufficient_tissues)) {
  tissue <- sufficient_tissues[i]
  cat("\n[", i, "/", length(sufficient_tissues), "]")
  
  result <- run_tissue_analysis(
    tissue_name = tissue,
    droplet_data = droplet,
    smartseq_data = smartseq,
    num_cores = 20,
    verbose = TRUE
  )
  
  qsave(result, paste0('save_data_v3/',tissue, '_intermediate_results.qs'))
  all_results[[tissue]] <- result
  
  # Save intermediate results
  # qsave(all_results, 'save_data_v3/all_tissues_intermediate_results.qs')
}

# Save final results
qsave(all_results, 'save_data_v3/all_tissues_final_results.qs')
...
create_summary_report <- function(results_list) {
  summary_df <- map_dfr(results_list, function(result) {
    data.frame(
      ...
      n_cells_10x = ifelse(is.null(result$data_info), NA, result$data_info$n_cells_10x),
      n_cells_smartseq = ifelse(is.null(result$data_info), NA, result$data_info$n_cells_smartseq),
      common_cell_types = ifelse(is.null(result$data_info), NA, result$data_info$common_cell_types),
      best_nmi = ifelse(is.null(result$results) || is.null(result$results$best_model), 
                        NA, 
                        result$results$best_model@results$nmi),
      analysis_time = ifelse(is.null(result$results) || is.null(result$results$analysis_time), 
                             NA, 
                             result$results$analysis_time$total_time)
    )
  })
...
write.csv(summary_report, 'save_data_v3/tissue_analysis_summary.csv', row.names = FALSE)
...
completed <- sum(summary_report$status == "completed")
errors <- sum(summary_report$status == "error")
skipped <- sum(summary_report$status == "skipped")
loaded <- sum(summary_report$status == "loaded")

cat("\nFinal Statistics:\n")
cat("  Completed:", completed, "\n")
cat("  Loaded from cache:", loaded, "\n") 
cat("  Skipped:", skipped, "\n")
cat("  Errors:", errors, "\n")
cat("  Total tissues processed:", length(all_results), "\n")

if (completed > 0) {
  avg_nmi <- summary_report %>% 
    filter(status == "completed") %>% 
    pull(best_nmi) %>% 
    mean(na.rm = TRUE)
  cat("  Average NMI for completed analyses:", round(avg_nmi, 3), "\n")
}